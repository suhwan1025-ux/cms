...
app.get('/api/proposals/:id', async (req, res) => {
  try {
    ...
    // ProposalCorrection 모델이 있으면 포함
    if (models.ProposalCorrection) {
      includeArray.push({
        model: models.ProposalCorrection,
        as: 'corrections',
        required: false
      });
    }
...
// 품의서 정정 저장
app.post('/api/proposals/:id/correction', async (req, res) => {
  const transaction = await sequelize.transaction();
  ...
  try {
    ...
    console.log('=== 품의서 정정 시작 ===');
    console.log('품의서 ID:', id);
    console.log('정정 내용:', corrections);
    console.log('정정 사유:', reason);

    // 품의서 확인
    const proposal = await models.Proposal.findByPk(id, { transaction });
    if (!proposal) {
      await transaction.rollback();
      return res.status(404).json({ error: '품의서를 찾을 수 없습니다.' });
    }

    // 작성완료 상태인지 확인 (여러 상태 허용)
    const allowedStatuses = ['approved', '작성완료', '결재완료', 'submitted'];
    if (!allowedStatuses.includes(proposal.status) || proposal.isDraft) {
      await transaction.rollback();
      return res.status(400).json({ error: '작성완료된 품의서만 정정할 수 있습니다. 현재 상태: ' + proposal.status });
    }
    ...
    try {
      const user = await getUserByIP(clientIP);
      if (user && user.name) {
        correctedBy = user.name;
      }
    } catch (error) {
      console.log('사용자 정보 조회 실패, 기본값 사용:', correctedBy);
    }

    // 정정 이력 저장 (ProposalCorrection 모델이 있는 경우에만)
    let correction = null;
    if (models.ProposalCorrection) {
      correction = await models.ProposalCorrection.create({
        proposalId: id,
      ...
      console.warn('⚠️ ProposalCorrection 모델이 없습니다. 정정 이력은 저장되지 않습니다.');
    }

    // 품의서 실제 값 업데이트
    const updateData = {};
    Object.entries(corrections).forEach(([field, values]) => {
    ...
    console.log('✅ 품의서 정정 완료');
    res.json({
      message: '품의서가 정정되었습니다.',
      correctionId: correction ? correction.id : null,
      proposal: proposal
    });
...
// 품의서 정정 이력 조회
app.get('/api/proposals/:id/corrections', async (req, res) => {
  try {
    const { id } = req.params;
    ...
    const corrections = await models.ProposalCorrection.findAll({
      where: { proposalId: id },
    ...
    console.error('정정 이력 조회 오류:', error);
    res.status(500).json({ error: error.message });
  }
});
