...
function OverallDashboard() {
  ...
  const fetchAllData = async () => {
    try {
      ...
      console.log('전산운용비 통계:', {
        selectedYear,
        전체건수: operatingBudgets.length,
        당해년도건수: currentYearOperatingBudgets.length,
        총예산: operatingTotalBudget,
        총집행: operatingTotalExecuted,
        집행률: operatingBudgetStats.executionRate
      });
      ...
      const personnelStats = {
        ...
        external: externalPersonnel.length,
        externalActive: externalPersonnel.filter(ep => ep.employment_status === '재직중').length
      };
