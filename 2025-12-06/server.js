...
app.get('/api/proposals', async (req, res) => {
  try {
    ...
    if (req.query.originalProposalId) {
      const originalProposalId = parseInt(req.query.originalProposalId);
      if (!isNaN(originalProposalId)) {
        // DB 컬럼명을 직접 사용 (snake_case)
        whereClause.original_proposal_id = originalProposalId;
        console.log('🔍 정정 품의서 조회 - 원본 ID:', originalProposalId);
      }
    }
