...
async function testEditRestoreFinal() {
  try {
    console.log('=== 편집 모드 데이터 복원 최종 테스트 ===');

    // 1. 테스트 데이터 생성
    const testData = {
      contractType: 'purchase',
      purpose: '편집 복원 최종 테스트',
      basis: '편집 복원 최종 테스트 근거',
      budget: 1,
      contractMethod: '일반계약',
    ...
    // 3. 편집 모드에서 복원되는 데이터 시뮬레이션 (실제 ProposalForm.js 로직)
    ...
    console.log(`예상 비용분배 개수: ${testData.purchaseItems.length * 2}개 (1개 품목 × 2개 부서)`);
    ...
    // 6. 성공/실패 판정
    const expectedAllocations = testData.purchaseItems.length * 2; // 1개 품목 × 2개 부서
