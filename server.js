...
app.post('/api/proposals', async (req, res) => {
  try {
    ...
    const createData = {
      ...
      correctionReason: proposalData.correctionReason !== undefined ? proposalData.correctionReason : null, // 정정 사유 (빈 문자열도 허용)
    ...
    console.log('✅ 품의서 생성 성공:', {
      ...
      originalProposalId: proposal.originalProposalId,
      correctionReason: proposal.correctionReason ? '있음' : '없음'
    });
    ...
    console.log('🔍 DB 저장 확인 (재조회):');
    console.log('  originalProposalId (camelCase):', savedData.originalProposalId);
    console.log('  original_proposal_id (snake_case):', savedData.original_proposal_id);
    console.log('  correctionReason (camelCase):', savedData.correctionReason);
    console.log('  correction_reason (snake_case):', savedData.correction_reason);
...
app.get('/api/proposals', async (req, res) => {
  try {
    ...
    const proposalsWithBudget = await Promise.all(proposals.map(async (proposal) => {
      ...
      // 🔧 정정 관련 필드 키 변환 (snake_case → camelCase)
      if (proposalData.original_proposal_id !== undefined && proposalData.originalProposalId === undefined) {
        proposalData.originalProposalId = proposalData.original_proposal_id;
      }
      if (proposalData.correction_reason !== undefined && proposalData.correctionReason === undefined) {
...
app.get('/api/proposals/:id', async (req, res) => {
  try {
    ...
    // 🔍 디버깅: toJSON 후 키 형식 확인
    console.log('🔍 [GET /api/proposals/:id] toJSON() 결과 키 확인:');
    console.log('  originalProposalId (camelCase):', proposalData.originalProposalId);
    console.log('  original_proposal_id (snake_case):', proposalData.original_proposal_id);
    console.log('  correctionReason (camelCase):', proposalData.correctionReason);
    console.log('  correction_reason (snake_case):', proposalData.correction_reason);
    
    // 🔧 명시적 키 변환: snake_case → camelCase (underscored 설정 때문에 필요할 수 있음)
    if (proposalData.original_proposal_id !== undefined && proposalData.originalProposalId === undefined) {
      proposalData.originalProposalId = proposalData.original_proposal_id;
      console.log('✅ original_proposal_id → originalProposalId 변환 완료');
    }
    if (proposalData.correction_reason !== undefined && proposalData.correctionReason === undefined) {
      proposalData.correctionReason = proposalData.correction_reason;
      console.log('✅ correction_reason → correctionReason 변환 완료');
    }
